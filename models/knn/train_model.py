...
def evaluate_on_samples(
	...
	if not sample_dirs:
		logging.warning("No validation samples available; skipping validation")
		return
...
def parse_args() -> argparse.Namespace:
	p = argparse.ArgumentParser(description="Train pixel-wise KNN (Euclidean) for land-use classification.")
	p.add_argument("--train-data", type=Path, default=Path(__file__).parent / "../../train_data", help="Dataset folder with sample subfolders.")
	p.add_argument("--model-out", type=Path, default=Path(__file__).parent / "knn_landuse_model.joblib", help="Output path for trained model.")
	return p.parse_args()
...
def main() -> None:
	...
	logging.info("Starting KNN (Euclidean) training pipeline")
	logging.info(
		"Config: train_data=%s val_ratio=%.2f n_neighbors=%d weights=%s train_pixel_fraction=%.3f seed=%d",
		args.train_data,
		VAL_RATIO,
		N_NEIGHBORS,
		WEIGHTS,
	...
	logging.info("Spectral features per pixel: %d (true+swir+ndvi RGB)", len(FEATURE_ORDER))
	...
	logging.info("Fitting KNN model (this stores training examples for nearest-neighbor search)...")
	fit_t0 = time.time()
	model.fit(x_train, y_train)
	logging.info("Model fit finished in %.2fs", time.time() - fit_t0)
