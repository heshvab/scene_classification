...
def build_feature_matrix(images: Dict[str, np.ndarray], feature_order) -> np.ndarray:
    ...
    if missing:
        raise RuntimeError(f"Model checkpoint expects features not provided by this script: {missing}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run pixel-wise land-use classification using trained Random Forest model.")
    p.add_argument("--model", type=Path, default=Path(__file__).parent / "rf_landuse_model.joblib", help="Path to trained model file.")
